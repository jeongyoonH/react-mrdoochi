...
class InstaexperScreenshot extends Component {
    state ={
        imgs : []
    }
    ...
    async componentDidMount(){
        ...
        // console.log(screendb['marketingType']['insta_experience']['image'])
				const username = JSON.parse(sessionStorage.auth).user.username;
				const name = this.props.name;
        const screendb = await getMarketingDetail(username, name,'');
        if(screendb !== null ) {
	        for (let val of screendb['marketingType']['insta_experience']['image']) {
...
export default InstaexperScreenshot;